...
class MenuInterface {
  ...
  void _iniciarDashboardBackground() {
    ...
    _dashboardSubs = sistemaController.streamDadosTempoReal().listen((data) {
      ...
      print('├' + '─' * 70 + '┤');
      print('│ 💡 Pressione ENTER para sair do dashboard${' ' * 25}│');
      print('└' + '─' * 70 + '┘');
    });
    ...
    while (contador < 10) {
      ...
      try {
        ...
        if (dados != null) {
          ...
          EstadoClimatizador? clima = sistemaController.ultimoEstadoClima;
          print('┌' + '─' * 70 + '┐');
          print(
          ...
          print('⚠️  Aguardando dados do Firebase...');
        }
        ...
        print('❌ Erro ao atualizar dashboard: $e');
  ...
  String _formatResumoSistema(Map<String, dynamic> data) {
    ...
    String tagsStr;
    if (tagsVal is List) {
      tagsStr = tagsVal.isEmpty ? 'Nenhuma tag detectada' : tagsVal.join(', ');
    } else {
      tagsStr = tagsVal.toString();
    }

    final temp = sensores != null && sensores['temperatura'] != null
        ? (sensores['temperatura'] as num)
                  .toDouble()
                  .toStringAsFixed(1)
                  .padLeft(5) +
              '°C'
    ...
    lines.add(_padInner('🏷️  Funcionários: ${tagsStr.padRight(35)}'));

    if (climatizador != null) {
      final ligado = climatizador['ligado'] == true
          ? '🟢 LIGADO  '
          : '🔴 DESLIGADO';
      final vel = climatizador['velocidade']?.toString().padLeft(2) ?? '-';
      final umid = climatizador['umidificando'] == true ? 'SIM' : 'NÃO';
      lines.add(_padInner('❄️  Clima: $ligado   Vent: $vel   Umidif: $umid'));
    } else {
      lines.add(_padInner('❄️  Climatizador: 🔴 DESCONECTADO${' ' * 25}'));
    }
  ...
  void _printDadosCard(DadosSensores dados, EstadoClimatizador? clima) {
    // linha de sensores
    print(
      _padInner(
    ...
    // tags
    print(_padInner('🏷️  Tags: ${dados.tags.join(', ')}'));

    // clima
    if (clima != null) {
      final status = clima.ligado ? '🟢 LIGADO' : '🔴 DESLIGADO';
      print(
        _padInner(
    ...
    try {
      ...
      if (funcionarios.isEmpty) {
        ...
        for (Funcionario func in funcionarios) {
          String matricula = func.matricula.toString().padRight(7);
          String nomeCompleto = '${func.nome} ${func.sobrenome}';
          String nome = nomeCompleto.length > 24
              ? '${nomeCompleto.substring(0, 21)}...'
              : nomeCompleto.padRight(24);
          String temp = func.tempPreferida.toStringAsFixed(1).padLeft(7);
          String lumi = func.lumiPreferida.toString().padLeft(7);
          String tag = (func.tagNfc ?? 'N/A').padRight(12);

          print('│ $matricula │ $nome │ $temp │ $lumi │ $tag │');
        }
  ...
  Future<void> _cadastrarFuncionario() async {
    ...
    try {
      ...
      if (lumiStr != null && lumiStr.trim().isNotEmpty) {
        ...
        if (lumi != null && lumi >= 0 && lumi <= 100) lumiPreferida = lumi;
      }
      ...
      if (resp != null && resp.trim().toLowerCase() == 's') {
        ...
        String? encontrada;
        final int maxAttempts = 20; // checa a cada 1s (20s timeout)
  ...
  Future<void> _editarFuncionario() async {
    ...
    try {
      ...
      Funcionario? funcionario = await funcionarioService
      ...
      if (novaLumi != null && novaLumi.trim().isNotEmpty) {
        ...
        if (lumi != null && lumi >= 0 && lumi <= 100) luminosidade = lumi;
      }
  ...
  Future<void> _excluirFuncionario() async {
    ...
    try {
      ...
      if (confirmacao == null ||
          !['s', 'S', 'sim', 'SIM'].contains(confirmacao.trim())) {
  ...
  Future<void> _relatorioDiario() async {
    ...
    try {
      ...
      List<LogEntry> logs = await logService.listarPorPeriodo(
        inicioHoje,
        fimHoje,
      );
    ...
    try {
      ...
      switch (opcao) {
        case '1':
          ...
          await firebaseService.enviarComandoClimatizador(
            novoStatus ? 'ligar:1' : 'desligar',
          );
          print('✅ Climatizador ${novoStatus ? "ligado" : "desligado"}!');
          break;

        case '2':
          stdout.write('Velocidade do ventilador (0-3): ');
          String? velStr = await _readLineAsync();
          int? novaVel = int.tryParse(velStr ?? '');
          if (novaVel != null && novaVel >= 0 && novaVel <= 3) {
            await firebaseService.enviarComandoClimatizador(
              'velocidade:$novaVel',
            );
            print('✅ Ventilador ajustado para velocidade $novaVel!');
          } else {
            print('❌ Velocidade inválida (deve ser 0, 1, 2 ou 3)!');
          }
          break;
  ...
  Future<void> _controlarIluminacao() async {
    ...
    try {
      ...
      if (intensidade != null && intensidade >= 0 && intensidade <= 100) {
        await firebaseService.enviarComandoIluminacao(intensidade);
  ...
  Future<void> _resetarSistema() async {
    ...
    if (confirmacao != null && confirmacao.toLowerCase() == 's') {
      try {
        ...
        bool sucessoClimaInterno = await sistemaController
            .enviarComandoClimatizador('auto');
        bool sucessoClimaFirebase = false;
        try {
          sucessoClimaFirebase = await firebaseService
              .enviarComandoClimatizador('auto');
        } catch (_) {
        ...
        bool sucessoIlum = await sistemaController.definirIluminacaoManual(
          'auto',
        );
          ...
          try {
            ...
            if (dados != null) {
              if (dados.pessoas == 0) {
                ...
                PreferenciasGrupo? prefs = await sistemaController
                    .processarSolicitacaoPreferencias(dados.tags);
                if (prefs != null) {
